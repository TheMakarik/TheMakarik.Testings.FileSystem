...
.AddFilesWithNameGeneraing(".json", 50)
```
...
**Пример:**
```csharp
.AddFilesWithNameGeneraing(".csv", 10)
.RefreshNameGenerator()
.AddFilesWithNameGeneraing(".log", 5)   // нумерация начнётся заново
```
...
- `AddFileWithNameGeneraing(string extension, string? content = null)`
- `AddFileWithNameGeneraing(string extension, out string fullPath, string? content = null)`
- `AddFilesWithNameGeneraing(string extension, int count, string? content = null)`
- `AddFilesWithNameGeneraing(string extension, out string[] fullPaths, int count, string? content = null)`
...
_fileSystem = FileSystem.BeginBuilding()
    ...
    .AddNameGenerator(NameGenerationType.RandomNameAndCount)
    .AddFilesWithNameGeneraing(".txt", 200, "Test line\n")
    .RefreshNameGenerator()
    .AddFilesWithNameGeneraing(".json", 50, "{}")
    .Build();
...
_fileSystem = FileSystem.BeginBuilding()
    ...
    .AddFilesWithNameGeneraing(".db", 30)
    .Build();
```
