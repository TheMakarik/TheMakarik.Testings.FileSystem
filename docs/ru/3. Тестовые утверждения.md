...
Вы можете использовать свойство `No`, для того чтобы заставить ассерты падать при правильных результатах, и проходить при невалидных.
...
## Проверки структуры корневой директории

### 1) Проверка точного соответствия содержимого корневой директории
```csharp
public static IFileSystemAssertion ContentEquals(
    this IFileSystemAssertion assertion,
    IEnumerable<string> expectedItems)
```
Проверяет, что корневая директория содержит ровно указанные файлы и поддиректории. Сравнение выполняется по именам без учета регистра.
...
### 2) Проверка что корневая директория содержит только файлы
```csharp
public static IFileSystemAssertion ContentContainsOnlyFiles(
    this IFileSystemAssertion assertion)
```
Проверяет, что в корневой директории отсутствуют поддиректории. Не проверяет количество или имена файлов.

**Пример:**
```csharp
// Проверяем что в корне нет подпапок (только файлы)
_fileSystem.Should().ContentContainsOnlyFiles();
```

### 3) Проверка что корневая директория содержит только поддиректории
```csharp
public static IFileSystemAssertion ContentContainsOnlyDirectories(
    this IFileSystemAssertion assertion)
```
Проверяет, что в корневой директории отсутствуют файлы. Не проверяет количество или имена поддиректорий.

**Пример:**
```csharp
// Проверяем что в корне только папки (например, проект только с модулями)
_fileSystem.Should().ContentContainsOnlyDirectories();
```

### 4) Проверка общего размера файловой системы
```csharp
public static IFileSystemAssertion TotalSizeLessThan(
    this IFileSystemAssertion assertion,
    long maxSizeInBytes)
```
Проверяет, что общий размер всех файлов в файловой системе (включая вложенные папки) не превышает указанный лимит.

**Пример:**
```csharp
// Проверяем что весь проект занимает меньше 10MB
_fileSystem.Should().TotalSizeLessThan(10 * 1024 * 1024);
```

### 5) Проверка общего количества файлов
```csharp
public static IFileSystemAssertion TotalFileCount(
    this IFileSystemAssertion assertion,
    int expectedFileCount)
```
Проверяет точное количество файлов во всей файловой системе (рекурсивно включая все поддиректории).

**Пример:**
```csharp
// Проект должен содержать ровно 42 файла
_fileSystem.Should().TotalFileCount(42);
```

### 6) Проверка отсутствия дубликатов имен файлов
```csharp
public static IFileSystemAssertion NoDuplicateFileNames(
    this IFileSystemAssertion assertion)
```
Проверяет, что все файлы в файловой системе имеют уникальные имена (с учетом регистра).

**Пример:**
```csharp
// Убеждаемся что нет файлов с одинаковыми именами
_fileSystem.Should().NoDuplicateFileNames();
```

### 7) Проверка шаблонов имен файлов
```csharp
public static IFileSystemAssertion AllFilesMatchNamePattern(
    this IFileSystemAssertion assertion,
    string namePattern)
```
Проверяет, что все файлы соответствуют указанному регулярному выражению. Проверяется только имя файла (без расширения).

**Пример:**
```csharp
// Все файлы должны начинаться с префикса "module_"
_fileSystem.Should().AllFilesMatchNamePattern("^module_.*$");
```

### 8) Поиск файла с определенным содержимым
```csharp
public static IFileSystemAssertion ContainsFileWithText(
    ...
    StringComparison stringComparison = StringComparison.Ordinal)
```
Проверяет, что хотя бы один файл в файловой системе содержит указанный текст.

**Пример:**
```csharp
// Проверяем что где-то в проекте есть TODO комментарий
_fileSystem.Should().ContainsFileWithText("TODO");
```

### 9) Проверка максимальной глубины вложенности
```csharp
public static IFileSystemAssertion MaxDirectoryDepth(
    this IFileSystemAssertion assertion,
    int maxDepth)
```
Проверяет, что глубина вложенности директорий не превышает указанного значения.

**Пример:**
```csharp
// Ограничиваем вложенность 3 уровнями
_fileSystem.Should().MaxDirectoryDepth(3);
```

### 10) Проверка соглашений об именовании
```csharp
public static IFileSystemAssertion FollowsNamingConvention(
    this IFileSystemAssertion assertion,
    Func<string, bool> conventionValidator)
```
Позволяет проверить, что все элементы в корневой директории следуют определенному правилу именования.

**Пример:**
```csharp
// Проверяем что все имена в нижнем регистре
_fileSystem.Should().FollowsNamingConvention(name => 
    name == name.ToLowerInvariant());

// Проверяем что нет файлов с пробелами
_fileSystem.Should().FollowsNamingConvention(name => 
    !name.Contains(' '));
```
...
### Пример 1: Проверка структуры проекта
```csharp
[Fact]
...
### Пример 2: Проверка лимитов сборки
```csharp
[Fact]
...
### Пример 3: Проверка соглашений кодирования
```csharp
[Fact]
public void Codebase_ShouldFollowNamingConventions()
{
    _fileSystem.Should()
        ...
        .ContentContainsOnlyDirectories(); // src папка должна содержать только директории
}
```
