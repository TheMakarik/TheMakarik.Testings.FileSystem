...
### Автогенерация имён tar-архивов

При активном авто-нейминге (`AddNameGenerator`) можно генерировать имена и для tar-архивов.
...
#### 1) Один tar-архив с автонеймингом

```csharp
var fs = FileSystem.BeginBuilding()
    .AddRandomInTempRootName()
    .AddNameGenerator(NameGenerationType.RandomNameAndCount)
    .AddTarWithNameGenerating(tar => tar
        .AddFile("readme.txt", "auto-named tar"))
    .Build();
```

Имя архива будет сгенерировано автоматически c учётом выбранного `TarPackTo`:

- `TarPackTo.None` → расширение `.tar`
- `TarPackTo.GZip` → `.tar.gz`
- `TarPackTo.BZip2` → `.tar.bz2`

#### 2) Один tar-архив с получением полного пути

```csharp
fs = FileSystem.BeginBuilding()
    .AddRandomInTempRootName()
    .AddNameGenerator(NameGenerationType.RandomNameAndCount)
    .AddTarWithNameGenerating(out var tarPath, tar => tar
        .AddFile("dump.sql", "sql"), TarPackTo.GZip)
    .Build();
```

При желании можно использовать более читаемые алиасы:

```csharp
fs = FileSystem.BeginBuilding()
    .AddRandomInTempRootName()
    .AddNameGenerator(NameGenerationType.RandomNameAndCount)
    .AddTarWithAutoNaming(out var tarPath, tar => tar
        .AddFile("dump.sql", "sql"), TarPackTo.GZip)
    .Build();
```

#### 3) Несколько tar-архивов

```csharp
fs = FileSystem.BeginBuilding()
    .AddRandomInTempRootName()
    .AddNameGenerator(NameGenerationType.RandomNameAndCount)
    .AddTarsWithNameGenerating(3, tar => tar
        .AddFile("data.txt", "backup"), TarPackTo.None)
    .Build();
```

#### 4) Несколько tar-архивов с путями

```csharp
fs = FileSystem.BeginBuilding()
    .AddRandomInTempRootName()
    .AddNameGenerator(NameGenerationType.RandomNameAndCount)
    .AddTarsWithNameGenerating(2, out var tarPaths, tar => tar
        .AddFile("log.txt", "logs"), TarPackTo.GZip)
    .Build();
```

Аналогично, для всех вариантов доступны алиасы:

- `AddTarWithAutoNaming(...)` / `AddTarWithAutoNaming(out string fullPath, ...)`
- `AddTarsWithAutoNaming(int count, ...)` / `AddTarsWithAutoNaming(int count, out string[] fullPaths, ...)`

Во всех случаях используется тот же механизм `NamingInfo`, что и для обычных файлов/директорий, поэтому группы архивов можно разделять через `RefreshNameGenerator()`.
